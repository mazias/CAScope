...
liveanddrawnewcleanzoom(
	...
	*/
	...
	const UINT32 plw,									// pixel-screen line width
	const unsigned int pnp,								// pixel-screen pitch // currently ignored
...
)
{
	...
	int hw = 50;//
	if (sfcm)
		hw = max(0, plw - ceil(sqrt(scsz)) * hlpz);		// histogram width
	
	int tma = 0;										// time available (as stored in pixel buffer)
	const int clsz = sizeof(CA_CT);						// memory size of one cell
	int pbs = scsz / hlzm;								// pixel-buffer size
	//if (scsz % hlzm)
	//	++pbs;// += scsz % hlzm;
	const int stst = 0;									// state shift (nr. of state bits shifted out / disregarded)

	vlzm = max(vlzm, vlfs);								// vertical zoom can not be larger than zomm since it not possible (atm) to go back to previous generatios=lines
	if (vlfs == 0 || hlfs == 0)
		hlfs = vlfs = 0;
	...
	int brsz = max(hlfs - 1, cr->ncn - 1);	// 32 to make sure simd instructions have enough room
	brsz += 32;  // !! ??? Why is max 32 not enough???
	brsz = max(16 * 1024, brsz);
	...
	if (sfcm)
		...
		int vlsl = tm / max(1, scsz * vlzm) * vlpz;				// vertical scroll / lines to scroll
		tm = vlsl * vlzm * scsz / vlpz;// tm / (vlzm * scsz) * (vlzm * scsz);
		...
		else if (vlsl * plw < pni - pnv) {
			pnc = max(pnv, pni - vlsl * plw - hlsl);			// set cursor to first pixel that has to be drawn
	...
	if (!csv | res) {
		...
		csv = _aligned_malloc((scsz + brsz) * clsz, 4096);
		csi = csv + scsz;
	...
	UINT64 dyss;
	if (ds.fsme == 0)
		dyss = max(1, (cr->ncs * (max(1, max(1, hlfs) * max(1, vlfs) * max(1, tmfs + 1)))) >> stst);
	else if (ds.fsme == 1)
		dyss = max(1, ipow(cr->ncs, hlfs * vlfs * (tmfs + 1)) >> stst);
	...
	while (1) {
		...
		if (de && pbc < pbi) {// has been - pbi - pbc > 0) { - has this a reason?
			...
			*/
					...
					int cs =
						(
							...
							max(0,
								min(
									(pni - pnc),
									hlpz * (pbi - pbc)))));		// size of copy
						//printf("pn %d  pb %d   cs %d  plw %d\n", (pni-pnc), pbi-pbc,cs, plw);
					if (cs > 0) {
						if (!sm && pbs * hlpz > plw)
							pbc = pbv + (pbs * hlpz - plw) / 2 / hlpz;
						if (!sm && pbs * hlpz < plw)
							pnc += (plw - pbs * hlpz) / 2;
						...
						if (hlpz > 1) {
							...
							{
								for (int hi = 0; hi < cs / hlpz; ++hi) {
									for (int hzi = 0; hzi < hlpz; ++hzi) {
										*pnc = *pbc;
										++pnc;
									}
									++pbc;
			...
			else if (sfcm >= 1 && sfcm <= 4) {
				display_2d_matze(pnv + hw, pni, plw, plw - hw, vlpz, hlpz, pbv, pbi, sfcm - 1);
				pbc = pbi;
				de = 0;
			}
			else if (sfcm > 4 && sfcm < 5 + LMS_COUNT) {
				display_2d_lindenmeyer(LMSA[sfcm - 5], pnv + hw, pni, plw, plw - hw, vlpz, hlpz, pbv, pbi, sfcsw, -1, NULL, NULL, NULL);
				pbc = pbi;
				de = 0;
			}
			else {
				display_2d_chaotic(pnv, pni, plw, csv, csi, pbv, pbi, res);
			}
		}
		...
		for (int vlp = 0; vlp < vlzm; ++vlp) {
			...
			if (de && (vlp < vlfs || vlp == 0)) {
				/* Count states */
				if (tmfs == 0) {
					ca_count__simd(csv, csf, csi, pbv, pbi, hlzm, hlfs, ds.fsme, cr->ncs);
				}
				/* Accumulate states */
				else {
					static ic = 0;
					...
					ic++;
					//
					ca_count__simd(csv, csf, csi, abv, abi, hlzm, hlfs, ds.fsme, cr->ncs);
					/* Accumulate states */
					if (tmfs > 0) {
			...
			for (int i = 0; i < dyss; ++i) {
				...
				for (int v = sbt; v <= sbb; ++v) {
					...
					if (md == 0)
						hx = hw;
					for (int h = 0; h < hx; ++h) {
						if (pnv[v * plw + h] == 0x000000)
							pnv[v * plw + h] = 0xFFFFFF;
						else
							pnv[v * plw + h] = 0x0;
					}
				}
		...
		for (int i = 0; i < dyss; ++i) {
			...
			while (1) {
				...
				for (int vv = min(v, lv); vv <= max(v, lv); ++vv) {
					...
					if (pnv[vv * plw + x] < 0x800000)
						pnv[vv * plw + x] = 0xFFFFFF;
					else
						pnv[vv * plw + x] = 0x0;
				}
				lv = v;
...
CA_MAIN(void) {
	...
	int stzm = 1;				// state zoom - 0 = direct zoom on intensity
	int sfcm = 1;				// screen-filling-curve-mode
	int sfcsw = 1;				// screen-filling-curve-step-width
	int hlfs = 1;				/* focus / sharpening mode */
	int tmfs = 1;				/* time focus / sharpening mode */
	int vlfs = 1;				/* focus / sharpening mode */
	int vlzm = 1;				/* vertical zoom */
	int hlzm = 1;				/* horizontal zoom */
	int vlpz = 1;				/* vertical pixel zoom */
	int hlpz = 1;				/* horizontal pixel zoom */
	...
	while (1) {
		...
		while (SDL_PollEvent(&e)) {
			...
			if (e.type == SDL_WINDOWEVENT) {
				...
				case SDLK_PLUS:
					if (ctl)
						hlfs = vlfs = hlzm = vlzm = min(hlzm, vlzm) + 1,
						fscd = 1;
					else if (sft)
						hlpz = vlpz = min(hlpz, vlpz) + 1,
						fscd = 1;
					else if (alt)
