...
CA_Rule(CA_RULE prl) {
	...
	/* Number of rules */
	prl.nrls = ipow(prl.ncs, prl.nns); // TODO does not behave well due to integer overflow on high numbers
	if (prl.nrls == 0)
		prl.nrls = MAXUINT64;
...
typedef struct caDisplaySettings {
	...
	int tm;											// test mode - 0: disabled, 1: enabled
	...
	unsigned int pnp;								// pixel-screen pitch // currently not used
	...
	)
{
	...
	// render		- only whole cell-spaces / lines;	when enough cells have been analyzed, they are rendered
