...
void
render_ca_line(
	UINT32 *sctbl,		// state count table
	double *dsctbl,		// double state count table
	double *tdsctbl,
...
)
{
	...
	int stst = 0;
	if (ds.fsme == 0)
		dyss = max(1, (cr->ncs * (max(1, max(1, ds.hlfs) * max(1, ds.vlfs) * max(1, ds.tmfs + 1)))) >> stst);
	else if (ds.fsme == 1)
		dyss = max(1, ipow(cr->ncs, ds.hlfs * ds.vlfs * (ds.tmfs + 1)) >> stst);
	...
	while (1) {
		...
		if (de ) {
			//render_ca_line(sctbl, dsctbl, tdsctbl, ds, pbv, pbi, pbc, dyss, res);
			static UINT32 *cltbl = NULL;						// color table
			if (!cltbl || res) {
				free(cltbl);
				cltbl = malloc(dyss * sizeof *cltbl);
