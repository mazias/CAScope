...
void buildSineWave(float *data, Uint32 length) {
	Uint32 i;
	int na = 1; // nr of amplitudes in length
	for (i = 0; i < length; i++)
		data[i] = sin(i*(Tao / length));
		//data[i] = (1.0 / getWaveformLength(0) * na * (i % (getWaveformLength(0) / na)));// (sin(i*(Tao / length)));
		//data[i] = 0.2;// (1.0 / getWaveformLength(0) * na * (i % (getWaveformLength(0) / na)));// (sin(i*(Tao / length)));
}
...
int sound_main(int argc, char *argv[]) {
	...
	float* sineWave;
	sineWave = malloc(C0waveformLength * sizeof(*sineWave));
	...
	logVoice(&testVoiceA);
	logWavedata(sineWave, C0waveformLength, 100);
