...
#define HCTMXLV	62							// hash-cell-table-max-level
...
HCI hc_ens[HCTMXLV] = { 0 };				// hash-cell empty nodes
...
HC_STATS hc_stats[HCTMXLV + 2] = { 0 };		// hash-cell-stats - the two extra values are needed to store delta and sum statistics
...
void HC_print_stats() {
	...
	for (int i = 0; i < HCTMXLV; ++i) {
		...
		if (i && hc_stats[i].nc <= 3 && !hc_stats[i].sc && i < HCTMXLV)
			continue;
		double avgsk;
		if (hc_stats[i].ss == 0 || abs(hc_stats[i].sc) == 0)
			avgsk = 0.0;
		else
			avgsk = (double)hc_stats[i].ss / ((double)hc_stats[i].sc);
...
void HC_print_node(int ll, int mxll, HCI n) {
	...
	if (ll < 0) {
		...
		printf("%s#%2d/%08X  uc %d", indent, ll, n, hct[n].uc & HCUCMK);
		printf("\n");
...
HCI HC_find_or_add_branch(UINT32 ll, HCI ln, HCI rn, HCI* rt) {
	...
	while (1) {
		...
		if (hct[cm].ln) {
			...
			// Unused node found
			else if ((hct[cm].uc & HCUCMK) == 0) {
				UINT32 cnll = (hct[cm].uc & HCLLMK) >> 24;	// current-node-level
				// mark for recycling; prefer to recycle nodes with a higher level
				if (cnll > enll) {
					encm = cm;
					enll = cnll;
				}
		...
		if (!hct[cm].ln) {
			...
			HCI fm, fmr, sl, slr, sr, srr, tb, lrt, lrn, rrt, rln;
			...
			fm = HC_find_or_add_branch(ll - 1, lrn, rln, &fmr); // middle-result
			// Second pass.
			sl = HC_find_or_add_branch(ll - 1, lrt, fmr, &slr);
			sr = HC_find_or_add_branch(ll - 1, fmr, rrt, &srr);
			// Third pass.
...
> you can use hct[result_node].uc--;  to dereference uc for last returned node
*/
...
#define D 0				// debug-mode
	...
	memset(indent, ' ', HCTMXLV * 2);
	indent[ll * 2] = 0;
	printf("%sO L N l %d  n 0x%04X  uc %d            (press any key to continue)\n", indent, ll, n, hct[n].uc & HCUCMK);
	getch();
#endif
...
HCI convert_bit_array_to_hash_array(caBitArray* vba, int* ll) {
	...
	if (HCTBS == 4) {
		for (int sz = vba->scsz / 8 / 1; bi < sz; ++bi) {
			int l = 0;							// level
			UINT8* bya = vba->v;				// need to adress on byte-basis
			HC_add_node(0, 1 + ((bya[bi] >> 0) & 0x3), &l);
			HC_add_node(0, 1 + ((bya[bi] >> 2) & 0x3), &l);
			HC_add_node(0, 1 + ((bya[bi] >> 4) & 0x3), &l);
...
//__declspec(noinline)
UINT32*
display_hash_array(UINT32* pbv, UINT32* pbf, UINT32* pbi, int pll, HCI n, int dymd, UINT32* pmxv, UINT32* pmnv) {
	//	printf("xxx");
	...
	//printf("%d %d, ", ll, pll);
	while (ll <= pll) {
		if ((hddh >= 0 && ll >= hddh + hdll) || ll == hddh * -1 - 1 + hdll) {
			switch (dymd) {
			case 0: v += (hct[n].uc & HCUCMK);						break;
			case 1: v += (hct[n].uc & HCUCMK) << (pll - ll);		break;
			case 2: v += (hct[n].uc & HCUCMK) << (ll - hdll);		break;
			case 3: v *= (hct[n].uc & HCUCMK);						break;
			}
		}
...
void HC_init_base_nodes(CA_RULE cr) {
	...
	if (HCTBS == 4) {
		...
		while (1) {
			...
			hct[cm].uc = 1;		// level is 0, so we never want it do be deleted/reused, even if it is currently unused
			hct[cm].ln = ln;
			hct[cm].rn = rn;
			hct[cm].r = rt;
			hc_stats[0].nc++;
...
int64_t CA_CNFN_HASH(int64_t pgnc, caBitArray* vba) {
	...
	if (sdmrpt) {
		...
		if (hcfl) {
			...
			hc_sn = hc_ens[hc_sl];
			hct[hc_sn].uc++;
			SIMFW_SetFlushMsg(&sfw, "ERROR! Hash-table-size insufficient! ABORTING! Hash-cell-table has been CORRUPTED and will be cleared now!\n");
		}
		else {
			hct[hc_sn].uc--;
			hc_sn = rtnd;
...
void CA_CNITFN_HASH(caBitArray* vba, CA_RULE* cr) {
	...
	// no cell-space conversion for very large space-sizes
	if (vba->scsz >= 1 << 26) {
		hc_sl = (int)log2((double)vba->scsz) - 2; // hash-table-size-level
		printf("space-size very large (%llu) > using empty hash-node (level %d) for initialization\n", vba->scsz, hc_sl);
		hc_sn = hc_ens[hc_sl];
	}
	else {
		convert_CA_CT_to_bit_array(vba->clsc, (FBPT*)vba->v, vba->scsz);
	...
	// init with empty node the same size as vba->scsz
	//hc_sl = (log2fix(vba->scsz, 1) >> 1) - 1;
	//hc_sn = hc_ens[hc_sl];
	//hct[hc_sn].uc++;
	//HC_print_node(hc_sl, hc_sl, hc_sn);
...
display_2d_matze(
	...
	// Spiral-Layout
		...
		int d = 0;	// direction
		int l = 1;	// length
		int p = 0; // pos regarding to length
		UINT32 y = ceil(sqrt(pbs) / 2);
		UINT32 x = ceil(sqrt(pbs) / 2);
		for (UINT32 i = 0; i < cs; ++i) {
			UINT32* pnc = pnv + cr + y * vlpz * plw + x * hlpz;				// pixel-screen current element
			if (pnc >= pnv && pnc < pni)
				for (int v = 0; v < vlpzplw; v += plw)
...
)
{
	...
	/* Disable use of cell-space buffer for very large space-sizes */
	int csd = 0;												// cell-space-disabled
	if (ds.fsme == 2 && scsz >= 1 << 26) {
		csd = 1;
		if (ds.tm == 2) {
			ds.tm = 0;
			printf("WARNING test-mode can not be used - space-size too big!\n");
		}
	}
...
#define LOGSCALE (1U<<LOGPRECISION)
				...
				UINT32 mxv;														// max v
				UINT32 mnv;														// min v
				double rgv = 1.0;												// range v

				display_hash_array(pbv, pbv, pbi, hc_sl, hc_sn, ds.hdmd, &mxv, &mnv);
				//mxv = 0;
				//mnv = MAXUINT32;
				...
				//		mnv = *pbc;
				//}
//				printf("mnv %d  mx %u / %.2f%%\n", mnv, mxv, 100.0 / MAXUINT32 * mxv);

				if (lgmd)
					rgv = max(1.0, log2(mxv - mnv + 1));
				else
					rgv = max(1.0, mxv - mnv);
					...
					if (*pbc == lpbc)
						*pbc = lcol;
						...
						if (lgmd) {
							...
							v = log2((double)(*pbc - mnv + 1)) / rgv;
						}
						else
							v = (double)(*pbc - mnv) / rgv;
						lpbc = *pbc;
						*pbc = crtl[(int)floor(v * (CRTLSZ - 1) + .5)];
						lcol = *pbc;
					}
				}
				de = 0;
...
CA_MAIN(void) {
	...
	/* Init key-bindings */
	{
		...
		//
		nkb = eikb;
		nkb.name = "hash-display-mode";
		nkb.val = &ds.hdmd;
		nkb.slct_key = SDLK_t;
		nkb.min = 0; nkb.max = 3; nkb.wpad = 1;
		SIMFW_AddKeyBindings(&sfw, nkb);
		//
		nkb = eikb;
		...
		while (SDL_PollEvent(&e)) {
			...
			if (e.type == SDL_WINDOWEVENT) {
				...
				if (keysym >= SDLK_0 && keysym <= SDLK_9) {
					if (cnmd == CM_HASH) {
						...
						if (hcfl) {
							...
							hc_sn = hc_ens[hc_sl];
							hct[hc_sn].uc++;
							SIMFW_SetFlushMsg(&sfw, "ERROR! Hash-table-size insufficient! ABORTING! Hash-cell-table has been CORRUPTED and will by cleared now!\n");
						}
					}
					else {
					...
					if (cnmd == CM_HASH) {
						if (sft) {
							...
							while (1) {
								...
								for (HCI i = HCTBS + 1; i < HCISZ; i++) {			// go through all nodes, except the base-nodes
									if (hct[i].ln && !(hct[i].uc & HCUCMK)) {
										...
										maxuc = max(maxuc, (hct[i].uc & HCUCMK));
								}
								pc++;
								tdc += cdc;
								if (sft || !cdc)
							...
							while (1) {
								...
								printf("base node  %2d  cm %8X  ln %8X  rn %8X  rt %8X  uc %d\n", i + 1, cm, ln, rn, hct[cm].r, hct[cm].uc & HCUCMK);
							...
							*/
							...
							SIMFW_SetFlushMsg(&sfw, "removed unused nodes\nchecked  %d  hash-cells\npasses   %d\ndeleted  %d\nmaxuc    %d  %.2f%%", HCISZ, pc, tdc, maxuc, 100.0 / (double)HCUCMK * (double)maxuc);
						}
					}
					else {
						tm = 0.0;
				...
				case SDLK_PLUS:
					if (sft)
						...
						if (hc_sl < HCTMXLV - 1) {
							...
							if (ctl) {
								// empty node with the same size as current ca
								en = hc_ens[hc_sl];
							}
							else
				...
				if (HCISZPTLV != HCISZPT) {
					...
					hct = _aligned_malloc(HCISZ * sizeof(HCN), 256);
					if (!hct) {
						SIMFW_Die("malloc failed at %s (:%d), requested size %e\n", __FUNCSIG__, __LINE__, (double)HCISZ * sizeof(HCN));
						return;
					}
